...
class SpecNCutout(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.hole_fill == "min":
            ...
            self.hf = tf.random.uniform(input_shape, 0.0, 1.0, tf.float32)
        elif self.hole_fill == "random_with_range":
            self.hf = tf.random.uniform(
                input_shape, self.filler_min, self.filler_max, tf.float32
            )

    def generate_single_mask(
        self, h_start=int(), h_end=int(), w_start=int(), w_end=int()
    ):
        # 1d mask
    ...
    def generate_mixed_mask(
        self, bsz, hole_act_prob, hole_minh, hole_maxh, hole_minw, hole_maxw
    ):
        ...
        m_arr = tf.TensorArray(
            tf.bool, size=bsz, dynamic_size=False, clear_after_read=True
        )
        for b in tf.range(bsz):
            ...
            for n in tf.range(self.n_holes):
                if tf.random.uniform([]) < hole_act_prob:
                    _mask = self.generate_single_mask(
                        ys_start[b, n], ys_end[b, n], xs_start[b, n], xs_end[b, n]
                    )
        ...
        return (
            m_arr.stack()
        )  # shape: (b,h,w,1) with True for HOLE area (background is False)
    ...
    def call(self, x):
        ...
        if self.prob > 0:
            ...
            if self.uniform_mask:
                ...
                uni_mask = self.generate_mixed_mask(
                    1, 1.0, hole_minh, hole_maxh, hole_minw, hole_maxw
                )  # shape: (b,h,w,1)

                # Random activation within batch
                act_mask = tf.cast(
                    (tf.random.uniform([bsz, 1, 1, 1], 0, 1) < self.prob), tf.float32
                )
                ...
                x_aug = x_aug + self.get_true_holes(
                    uni_mask
                ) * act_mask * self.get_hole_filler(x)

                # Merge
                x = x_org + x_aug
            else:
                # Generate different masks for elements with independent prob
                masks = self.generate_mixed_mask(
                    bsz, self.prob, hole_minh, hole_maxh, hole_minw, hole_maxw
                )

                # Apply different masks
                x = x * self.get_true_background(masks) + self.get_true_holes(
                    masks
                ) * self.get_hole_filler(x)
            return x
        else:
            # bypass...
            return x
...
def test():
    ...
    audio, sampling_rate = librosa.load(
        "neural-audio-fp-dataset/music/train-10k-30s/"
        + "fma_small_8k_plus_medium_2k/000/000002.wav"
    )
    ...
    # Apply Cutout
    spec_ncutout_layer = SpecNCutout(
        prob=1.0,
        n_holes=1,
        uniform_mask=False,
        hole_fill="min",
