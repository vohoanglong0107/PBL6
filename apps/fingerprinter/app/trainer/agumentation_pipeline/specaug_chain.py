...
def get_specaug_chain_layer(cfg, trainable=False):
    ...
    """
    m = SpecAugChainer(
        chain_config=cfg["SPEC_AUG"]["SPECAUG_CHAIN"],
...
def test_chain():
    # Build chain
    input_mel = tf.keras.layers.Input(
        batch_size=64, shape=(128, 96, 1), name="input_mel"
    )
    augmented_mel = SpecAugChainer(
        chain_config=["cutout", "vertical", "horizontal"],
        probs=1.0,
        uniform_mask=True,
        n_holes=3,
        hole_fill="random",
    ...
    audio, sampling_rate = librosa.load(
        "neural-audio-fp-dataset/music/train-10k-30s/fma_small_8k_plus_medium_2k/000/000002.wav"
    )
    d = librosa.feature.melspectrogram(
        y=audio, sr=sampling_rate, n_mels=128, hop_length=128, fmax=8000
    )
    d = d[:, :96]
