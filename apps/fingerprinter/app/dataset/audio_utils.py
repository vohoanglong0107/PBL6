...
def log_scale_random_number_batch(bsz=int(), amp_range=(0.1, 1.0)):
    range_log = np.log10(amp_range)
    random_number_log = np.random.rand(bsz) * (
        np.max(range_log) - np.min(range_log)
    ) + np.min(range_log)
    return np.power(10, random_number_log)


def bg_mix_batch(
    event_batch, bg_batch, fs, snr_range=(6, 24), unit="db", mode="energy"
):
    ...
    event_amp_ratio_batch = log_scale_random_number_batch(
        bsz=len(event_batch), amp_range=(0.1, 1)
    )
    ...
    for i in range(len(event_batch)):
        ...
        if event_max == 0 or bg_max == 0:
            ...
            X_bg_mix[i] = background_mix(
                x=event_batch[i], x_bg=bg_batch[i], fs=fs, snr_db=snrs[i]
            )
...
def get_fns_seg_list(fns_list=[], segment_mode="all", fs=22050, duration=1, hop=None):
    """
